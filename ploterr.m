function hh = ploterr(x, y, xerr, yerr, LineWidth, varargin)
...
if ~isempty(xl) % x errorbars
    ...
    h = plotfct(barx,bary,esymbol,'LineWidth',LineWidth); hold on
end
if ~isempty(yl) % y errorbars
    [barx,bary]=barline(x,yl,yh,logx,hhy);
    h = [plotfct(barx,bary,esymbol,'LineWidth',LineWidth);h]; hold on
end
if ~isempty(y) % function values
    h = [plotfct(x,y,symbol,'LineWidth',LineWidth);h];
end
