function y = nanse(x,w,dim)
%   NANSE standard error, ignoring NaNs. Modifed from nanvar by SK
%   Y = NANSE(X) returns the sample standard error of the values in X, treating
%   NaNs as missing values.  For a vector input, Y is the standard error of the
%   non-NaN elements of X.  For a matrix input, Y is a row vector
%   containing the standard error of the non-NaN elements in each column of X.
%   For N-D arrays, NANSE operates along the first non-singleton dimension
%   of X.
%
%   NANSE normalizes Y by N-1 if N>1, where N is the sample size of the 
...
%
%   Y = NANSE(X,1) normalizes by N and produces the second moment of the
%   sample about its mean.  NANSE(X,0) is the same as NANSE(X).
%
%   Y = NANSE(X,W) computes the variance using the weight vector W.  The
...
%
%   Y = NANSE(X,W,DIM) takes the variance along dimension DIM of X.
%
